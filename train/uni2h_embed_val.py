"""Build the UNI2-h conditioning-embedding cache for the 1,427 DGX validation patches.
...
OUT_DIR = DATA_ROOT + "/DiffusionQC/dgx_shared/latents/uni2h_emb_val1427_rev-d517a8dd"
...
EXPECT_CLEAN, EXPECT_ARTIFACT = 1167, 260


def build_manifest():
    """Clean catalogue then artifact catalogue, each in its own stored order."""
    rows = []
    for split, name, expected in (("clean", "val_clean.json", EXPECT_CLEAN),
                                  ("artifact", "val_artifact.json", EXPECT_ARTIFACT)):
...
def main():
    ...
    log_run(OUT_DIR, {"uni_repo": UNI_REPO, "uni_revision": UNI_REVISION,
            ...
            inputs={"val_clean_catalogue": os.path.join(CATALOG_DIR, "val_clean.json"),
                    "val_artifact_catalogue": os.path.join(CATALOG_DIR, "val_artifact.json")})
    ...
    checks["V1_total_1427"] = len(manifest) == 1427
    ...
    checks["V4_shape_1427_16_1536"] = list(emb.shape) == [1427, N_TOKENS, D_MODEL]
    checks["V4_dtype_fp32"] = emb.dtype == torch.float32
    checks["V4_all_finite"] = bool(torch.isfinite(emb).all())
    checks["V5_payload_bytes"] = payload_bytes == 140279808

    meta = {
        "step": "3 -- UNI2-h conditioning embedding cache (input preprocessing)",
